	...
	<table class="layui-table" id="NavTable" lay-filter="NavTable">
		...
		<thead>
			<tr>
				...
				<th lay-data="{field:'borrowTime',width:'150',align:'center'}">借阅时间</th>
				<th lay-data="{field:'state',width:'150',align:'center'}">还书状态(0未还/1已还)</th>
				<th lay-data="{field:'backTime',width:'150',align:'center',sort:'true'}">还书时间</th>
				<!-- <th lay-data="{field:'tool',width:'200',fixed:'right',align:'center'}">操作</th> -->
			</tr>
		<tbody>
			<#list borrowrecords as record>
			<tr>
				<td lay-data="{checkbox:true,fixed:'left',align:'center'}"></td>
				<td>${record.bohuaiId}</td>
				<td>${record.uNo}</td>
				<td>${record.uName}</td>
				<td>${record.bookName}</td>
				<td>${(record.borrowTime?string("yyyy-MM-dd"))!}</td>
				<td>${record.state}</td>
				<td>${(record.backTime?string("yyyy-MM-dd"))!} </td>
				<!-- <td> -->
			<!-- 	<a href="${request.contextPath}/borrow/toedit?uNo=${borrow.uNo}"
					class="layui-btn layui-btn-sm " role="button">提醒
					<i class="layui-icon"></i></a> --> 
				<!-- <a href="${request.contextPath}/borrow/delete?id=${borrow.borrowId}&uno=${borrow.uNo}&bookname=${borrow.bookName}" 
						class="layui-btn layui-btn-sm layui-btn-danger"
					    role="button" onclick="return confirm('确定已归还???')"><i
